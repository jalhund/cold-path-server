...
local function mmul(a, b)
	local type = 'uint32_t'
	return tonumber(ffi_cast(type, ffi_cast(type, a) * ffi_cast(type, b)))
end

local function xxhash32(data, len, seed)
	seed, len = seed or 0, len or #data
	local i,n = 0, 0 -- byte and word index
	local bytes = ffi_cast('const uint8_t*', data)
	local words = ffi_cast('const uint32_t*', data)

	local h32
	if len >= 16 then
		local limit = len - 16
		local v = ffi_new("uint32_t[4]")
	...
	h32 = mmul(h32, P3)
	return tonumber(ffi_cast("uint32_t", xor(h32, shr(h32, 16))))
end
