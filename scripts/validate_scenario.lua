...
function M.validate(t, is_new_land)
    if not is_new_land and t.is_editor then
        return
    end
    ...
    -- Initialize AI enhancement toggles (default on for new games)
    if t.ai_memory_enabled == nil then
        t.ai_memory_enabled = true
    end
    if t.adaptive_difficulty_enabled == nil then
        t.adaptive_difficulty_enabled = false
    end
...
function M.make_sandbox(t)
    --debug_log("make sandbox", t.player_land,t.sandbox_mode)
    if t.sandbox_mode then
        t.lands[t.player_land].money = 999999999
        t.lands[t.player_land].science = 999999
        t.lands[t.player_land].skills = 180
        t.lands[t.player_land].intelligence = 999999
        t.lands[t.player_land].resources = {
            gold = 99999,
            uranium = 99999,
            heavy_water = 99999,
            weapons = 99999,
            chemical_weapon = 99999,
            tank = 99999,
            drone = 99999
        }
        for key, val in pairs(technology_data) do
            local lvl, n = key:match("t_(%d+)_(%d)")
            -- print("lvl: ", k, lvl, n, t.technology_lvl)
            table.insert(t.lands[t.player_land].opened_technology, key)
            for key_, val_ in pairs(val.bonuses) do
                if not val_[2] then
                    val_[2] = 0
                end
                val_[3] = "technology"
                table.insert(t.lands[t.player_land].bonuses, val_)
            end
        end
    end
end
